...
def process_result(result):
    ...
    try:
        ...
        print(f"评估结果时出错: {e}")
        return {"sample_id": result.get("sample_id", "unknown"), "error": str(e), "score": 0, "method": "error"}


def estimator_pass_at_k(n, c, k):
    """
    计算单个问题的 pass@k 无偏估计值
    公式来源: Chen et al. (2021) / Yue et al. (2025) Appendix A.2
...
#     # 计算多选和多图准确率
#     multi_choice_stats["accuracy"] = (
#         multi_choice_stats["correct"] / multi_choice_stats["total"] if multi_choice_stats["total"] > 0 else 0
#     )
#     multi_image_stats["accuracy"] = (
#         multi_image_stats["correct"] / multi_image_stats["total"] if multi_image_stats["total"] > 0 else 0
#     )
...
#         print(
#             f"多图样本准确率: {multi_image_stats['accuracy']:.4f} ({multi_image_stats['correct']}/{multi_image_stats['total']})"
#         )
...
if __name__ == "__main__":
    ...
    print(f"加载了 {len(results)} 条运行记录")

    # 1. 多进程评估每一条记录的正确性 (这一步还是按行处理，判断每条回答对不对)
    with multiprocessing.Pool(processes=args.num_workers) as pool:
        raw_evaluations = list(tqdm(pool.imap(process_worker, results), total=len(results), desc="评估答案正确性"))
    ...
    for eval_item in raw_evaluations:
        s_id = eval_item.get("sample_id", "unknown")

        # 记录基础信息 (假设同一 sample_id 的元数据一致)
        grouped_results[s_id]["category"] = eval_item.get("category", "unknown")
        grouped_results[s_id]["is_multi_image"] = eval_item.get("is_multi_image", False)
        grouped_results[s_id]["is_multi_choice"] = eval_item.get("is_multi_choice", False)

        # 添加本次运行的结果
        # 我们保存简化的运行信息以节省空间，或者保留完整信息
        # 这里为了后续分析方便，保留 'run_index' (如果有) 和详细预测
        eval_item_lite = {
            "run_index": eval_item.get("run_index", -1),  # 如果上一步没加run_index，这里可能是-1
            "predicted": eval_item.get("predicted"),
            "correct": eval_item.get("correct", False),
            "method": eval_item.get("method"),
        }
        grouped_results[s_id]["results"].append(eval_item_lite)
        grouped_results[s_id]["correctness_list"].append(eval_item["correct"])

    print(f"聚合后得到 {len(grouped_results)} 个唯一样本")

    # 3. 计算指标 (Pass@1, Pass@N 等)
    # 我们统计所有样本的 pass@k 值，然后取平均

    # 确定最大的采样次数 n_max
    max_n = 0
    for s_data in grouped_results.values():
        max_n = max(max_n, len(s_data["correctness_list"]))

    print(f"最大采样次数 (n): {max_n}")

    # 定义我们要计算的 k 值列表 (例如: 1, 4, 8, ... 直到 max_n)
    k_values = [k for k in [1, 2, 4, 8, 16, 32, 64, 128, 256] if k <= max_n]
    if max_n not in k_values:
        k_values.append(max_n)
    k_values.sort()

    # 初始化统计容器
    category_stats = defaultdict(lambda: defaultdict(list))  # category -> metric -> list of values
    overall_stats = defaultdict(list)  # metric -> list of values

    final_sample_details = []

    for s_id, data in grouped_results.items():
        n = len(data["correctness_list"])
        c = sum(1 for x in data["correctness_list"] if x)  # 正确次数
        ...
        sample_record = {
            ...
            "total_runs": n,
            "correct_count": c,
            "correctness_list": data["correctness_list"],  # === 你需要的list ===
            "pass_at_k": {},
            "runs_detail": data["results"],  # 包含每次运行的预测值
        }

        # 计算该样本在不同 k 下的 pass@k 贡献值 (0 到 1 之间)
        for k in k_values:
            if k <= n:
                score = estimator_pass_at_k(n, c, k)
                sample_record["pass_at_k"][f"pass@{k}"] = score

                # 添加到统计
                category_stats[data["category"]][f"pass@{k}"].append(score)
                overall_stats[f"pass@{k}"].append(score)

        final_sample_details.append(sample_record)

    # 4. 汇总结果
    # 计算平均值
    summary_results = {"overall": {}, "categories": {}}

    for k_key, scores in overall_stats.items():
        summary_results["overall"][k_key] = sum(scores) / len(scores) if scores else 0

    for cat, metrics in category_stats.items():
        summary_results["categories"][cat] = {}
        for k_key, scores in metrics.items():
            summary_results["categories"][cat][k_key] = sum(scores) / len(scores) if scores else 0
            # 记录该类别样本数
            summary_results["categories"][cat]["count"] = len(scores)

    # 5. 保存结果
    evaluation_dir = os.path.join(result_root_path, "evaluation_pass_k")  # 改个文件夹名以示区分
    os.makedirs(evaluation_dir, exist_ok=True)

    # 保存总体详细结果 (包含 correctness_list)
    total_details_file = os.path.join(evaluation_dir, "xlrsbench_samples_pass_k_details.jsonl")
    with open(total_details_file, "w", encoding="utf-8") as f:
        for record in final_sample_details:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")

    # 保存汇总结果
    eval_summary_file = os.path.join(evaluation_dir, "xlrsbench_pass_k_summary.json")
    with open(eval_summary_file, "w", encoding="utf-8") as f:
        json.dump(summary_results, f, ensure_ascii=False, indent=2)

    # 打印结果
    print("\n=== Pass@k 评测结果 ===")
    print(f"总样本数: {len(final_sample_details)}")
    print("总体表现:")
    for k in k_values:
        key = f"pass@{k}"
        if key in summary_results["overall"]:
            print(f"  {key}: {summary_results['overall'][key]:.4f}")

    print("\n结果已保存至:", evaluation_dir)
    print(f"  - 详细样本数据 (含正确性列表): {os.path.basename(total_details_file)}")
    print(f"  - 汇总指标: {os.path.basename(eval_summary_file)}")
